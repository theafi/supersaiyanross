...
<html>
	...
	<?php 
		...
		$pais = $_POST['Pais'];
		$telefono = $_POST['Ciudad'];

		$compemail =  "SELECT email FROM Usuarios WHERE email='$email';";
		$emailenlatabla = mysqli_query($conexion, $compemail);
		if (mysqli_num_rows($emailenlatabla) > 0) {
			echo "El email ya está registrado en la tabla";
		}
		else {
